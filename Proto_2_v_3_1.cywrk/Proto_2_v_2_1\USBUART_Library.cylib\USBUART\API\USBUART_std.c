    ...
    if ((CY_GET_REG8(`$INSTANCE_NAME`_bmRequestType) & `$INSTANCE_NAME`_RQST_DIR_MASK) == `$INSTANCE_NAME`_RQST_DIR_D2H)
    {
        ...
        case `$INSTANCE_NAME`_GET_DESCRIPTOR:
			...
			/* SN string*/
				...
				if( CY_GET_REG8(`$INSTANCE_NAME`_wValueLo) == `$INSTANCE_NAME`_DEVICE0_DESCR[`$INSTANCE_NAME`_DEVICE_DESCR_SN_SHIFT] )
				{
...
void `$INSTANCE_NAME`_Config()
{
    ...
    for (i = 0; i < 0x80; i+= 0x10)
    {
        ...
        `$INSTANCE_NAME`_EP[ep].wBuffOffset = bCount;
        bCount += `$INSTANCE_NAME`_EP[ep].wBufferSize;
...
uint8 `$INSTANCE_NAME`_SetEndpointHalt()
{
    ...
    if (ep != 0)
    {
        ...
        if (`$INSTANCE_NAME`_EP[ep].bAddr & `$INSTANCE_NAME`_DIR_IN)
        {
            ...
            CY_SET_REG8(&`$INSTANCE_NAME`_SIE_EP1_CR0[ri], (`$INSTANCE_NAME`_MODE_STALL_DATA_EP | `$INSTANCE_NAME`_MODE_ACK_IN));
        }
        else
        {
            /* OUT Endpoint */
            `$INSTANCE_NAME`_EP[ep].bAPIEPState = `$INSTANCE_NAME`_NO_EVENT_ALLOWED;
            CY_SET_REG8(&`$INSTANCE_NAME`_SIE_EP1_CR0[ri], (`$INSTANCE_NAME`_MODE_STALL_DATA_EP | `$INSTANCE_NAME`_MODE_ACK_OUT));
        }
        bRequestHandled = `$INSTANCE_NAME`_InitNoDataControlTransfer();
    }
    return bRequestHandled;
