...
extern uint8 CYCODE USBFS_HIDREPORT_DESCRIPTORS[];
...
{
    uint8 status = USBFS_IDLE_TIMER_INDEFINITE;
    ...
    if(USBFS_hidIdleRate[interface] != 0u)
    {
        if(USBFS_hidIdleTimer[interface] > 0u)
        {
            ...
            status = USBFS_IDLE_TIMER_RUNNING;
        }
        else
        {
            USBFS_hidIdleTimer[interface] = USBFS_hidIdleRate[interface];
            status = USBFS_IDLE_TIMER_EXPIRED;
        }
    }
        ...
        switch (*USBFS_bRequest)
        {
            ...
            case USBFS_HID_GET_IDLE:
                ...
                if( (interfaceNumber < USBFS_MAX_INTERFACES_NUMBER) && \
                       (CY_GET_REG8(USBFS_wValueLo) == 0u ) ) /* Report ID should be 0 */
                {
                    currentTD.count = 1u;
                    currentTD.pData = (uint8 *)&USBFS_hidIdleRate[interfaceNumber];
                    requestHandled  = USBFS_InitControlRead();
                }
                break;
            case USBFS_HID_GET_PROTOCOL:
                ...
                {
                    currentTD.count = 1u;
                    currentTD.pData = (uint8 *)&USBFS_hidProtocol[interfaceNumber];
                    requestHandled  = USBFS_InitControlRead();
                }
                break;
        ...
        switch (CY_GET_REG8(USBFS_bRequest))
        {
            ...
            case USBFS_HID_SET_IDLE:
                       ...
                       (CY_GET_REG8(USBFS_wValueLo) == 0u ) ) /* Report ID should be 0 */
                {
                    ...
                    USBFS_hidIdleTimer[interfaceNumber] = USBFS_hidIdleRate[interfaceNumber];
...
*
*******************************************************************************/
void USBFS_FindHidClassDecriptor()
{
    T_USBFS_LUT *pTmp;
    uint8 *pDescr;
    uint8 interfaceN;
...
*
*******************************************************************************/
void USBFS_FindReportDescriptor()
{
    T_USBFS_LUT *pTmp;
    uint8 *pDescr;
    uint8 interfaceN;
...
void USBFS_FindReport()
{
    ...
    pTmp = &pTmp[CY_GET_REG8(USBFS_wValueHi) - 1u];
