...
/*******************************************************************************
* Function Name: Clock_Signal_Out_Start
********************************************************************************
* Summary:
...
void Clock_Signal_Out_Start(void) 
{
    ...
    Clock_Signal_Out_CLKEN |= Clock_Signal_Out_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: Clock_Signal_Out_Stop
********************************************************************************
* Summary:
...
void Clock_Signal_Out_Stop(void) 
{
	...
	CYDEV_CHIP_REVISION_USED == CYDEV_CHIP_REVISION_5A_ES1))
/*******************************************************************************
* Function Name: Clock_Signal_Out_StopBlock
********************************************************************************
* Summary:
...
{
    if (Clock_Signal_Out_CLKEN & Clock_Signal_Out_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = Clock_Signal_Out_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Signal_Out_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_Signal_Out_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Signal_Out_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Clock_Signal_Out_StandbyPower
********************************************************************************
* Summary:
...
void Clock_Signal_Out_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Clock_Signal_Out_CLKSTBY &= ~Clock_Signal_Out_CLKSTBY_MASK;
    }
    else
    {
        Clock_Signal_Out_CLKSTBY |= Clock_Signal_Out_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Clock_Signal_Out_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Signal_Out_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = Clock_Signal_Out_GetSourceRegister();
    uint16 oldDivider = Clock_Signal_Out_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Signal_Out_CLKEN & Clock_Signal_Out_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Clock_Signal_Out_DIV_PTR, clkDivider);
                Clock_Signal_Out_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
        ...
        else
        {
            ...
            Clock_Signal_Out_CLKEN |= enabled;
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_Signal_Out_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(Clock_Signal_Out_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Signal_Out_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Signal_Out_SetModeRegister(uint8 clkMode) 
{
    Clock_Signal_Out_MOD_SRC |= clkMode & Clock_Signal_Out_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Signal_Out_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Signal_Out_ClearModeRegister(uint8 clkMode) 
{
    Clock_Signal_Out_MOD_SRC &= ~clkMode | ~Clock_Signal_Out_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_Signal_Out_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Clock_Signal_Out_MOD_SRC & Clock_Signal_Out_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Signal_Out_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Clock_Signal_Out_GetDividerRegister();
    uint8 oldSrc = Clock_Signal_Out_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_Signal_Out_MOD_SRC |= CYCLK_SSS;
        Clock_Signal_Out_MOD_SRC =
            (Clock_Signal_Out_MOD_SRC & ~Clock_Signal_Out_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Signal_Out_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Clock_Signal_Out_MOD_SRC & Clock_Signal_Out_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Signal_Out_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    Clock_Signal_Out_PHASE = clkPhase & Clock_Signal_Out_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Signal_Out_GetPhase
********************************************************************************
* Summary:
...
{
    return Clock_Signal_Out_PHASE & Clock_Signal_Out_PHASE_MASK;
}
