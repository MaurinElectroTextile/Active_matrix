...
//		ASSERT(0);
   ...
   /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REV_EXPECT <= CYDEV_CHIP_REV_LEOPARD_ES2) && (`$INSTANCE_NAME`_ep_0__ES2_PATCH))      
...
uint8 `$INSTANCE_NAME`_InitControlRead(void)
{
    ...
    wXferCount = (((uint16)*`$INSTANCE_NAME`_wLengthHi << 8) | (*`$INSTANCE_NAME`_wLengthLo));
...
uint8 `$INSTANCE_NAME`_InitControlWrite(void)
{
    ...
    wXferCount = (((uint16)CY_GET_REG8(`$INSTANCE_NAME`_wLengthHi) << 8) | (CY_GET_REG8(`$INSTANCE_NAME`_wLengthLo)));
...
void `$INSTANCE_NAME`_ControlWriteDataStage(void)
{
	uint8 *pReg = (uint8 *)`$INSTANCE_NAME`_EP0_DR0; 
